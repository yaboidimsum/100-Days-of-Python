...
#     # Group the data by the age_group and depression_level columns
#     age_group_depression_level_counts = df.groupby(['age_group', 'depression_level']).size().reset_index(name='counts')
    
#     # Calculate the mode (most frequent value) of depression_level for each age_group
#     modus_depression_level = age_group_depression_level_counts.loc[age_group_depression_level_counts.groupby('age_group')['counts'].idxmax()]
#     modus_depression_level = modus_depression_level[['age_group', 'depression_level']]
    
#     # Print the results
#     print('Modus Depression Level:')
#     print(modus_depression_level)
def depression_age_group_count(df):
    # Create the age_group column
    df['age_group'] = pd.cut(df['age'], bins=[17, 21, 25, 29, 100], labels=['18-21', '22-25', '26-29', '30 above'])
    df = df[df['depression_diagnosis'] == True]
