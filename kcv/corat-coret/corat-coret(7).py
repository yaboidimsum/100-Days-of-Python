...
def depression_age_group_count(df):
    ...
    age_group_depression_counts = df.groupby(['age_group', 'depression_level']).size().reset_index(name='counts')
    
    # # Print the results
    # print('Age Group and Depression Level Counts:')
    # for i, row in age_group_depression_counts.iterrows():
    ...
    print('Age Group and Depression Level Counts:')
    for age_group in age_group_depression_counts['age_group'].unique():
        group = age_group_depression_counts[age_group_depression_counts['age_group'] == age_group]
...
def anxiety_age_group_count(df):
    ...
    df.loc[df['gad_score'] >= 9, 'anxiety_level'] = 'Mild'
    df.loc[df['gad_score'] >= 14, 'anxiety_level'] = 'Moderate'
    df.loc[df['gad_score'] >= 21, 'anxiety_level'] = 'Moderately severe'
    # df.loc[df['gad_score'] >= 20, 'anxiety_level'] = 'Severe'
    
    # Group the data by the age_group and anxiety_diagnosis columns
    age_group_anxiety_counts = df.groupby(['age_group', 'anxiety_level']).size().reset_index(name='counts')
    
    # Print the results
    print('Age Group and Anxiety Level Counts:')
    for age_group in age_group_anxiety_counts['age_group'].unique():
        group = age_group_anxiety_counts[age_group_anxiety_counts['age_group'] == age_group]
        group = group.sort_values(by='counts', ascending=False)
        group = group.head(4)
        print(f'Age Group: {age_group}')
        for i, row in group.iterrows():
