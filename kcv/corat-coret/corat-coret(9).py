...
def anxiety_breakdown(df):
    # Run the anxiety_accuracy function to get the anxiety diagnosis
    anxiety_accuracy(df)

    # Get the number of people with anxiety
    anxiety_counts = df.groupby('anxiety_diagnosis').size().reset_index(name='counts')
    print('Anxiety Counts:')
    print(anxiety_counts)

    # Categorize the data based on anxiety severity, GAD-7 score, anxiousness, and anxious thoughts
    severity_counts = df.groupby(['anxiety_diagnosis', 'anxiety_severity']).size().reset_index(name='counts')
    gad_counts = df.groupby(['anxiety_diagnosis', 'gad_score']).size().reset_index(name='counts')
    anxiousness_counts = df.groupby(['anxiety_diagnosis', 'anxiousness']).size().reset_index(name='counts')

    # Print the results
    print('Anxiety Severity Counts:')
    print(severity_counts)
    print('GAD-7 Counts:')
    print(gad_counts)
    print('Anxiousness Counts:')
    print(anxiousness_counts)
...
def depression_age_group_count(df):
    ...
    # Group the data by the age_group, school_year, and depression_level columns
    age_group_depression_counts = df.groupby(['age_group', 'school_year', 'depression_level']).size().reset_index(name='counts')
    
    # Calculate the mean of the bmi for each age_group, school_year, and depression_level combination
    mean_bmi = df.groupby(['age_group', 'school_year', 'depression_level'])['bmi'].mean().reset_index(name='mean_bmi')
    
    # Merge the two dataframes
    age_group_depression_bmi = age_group_depression_counts.merge(mean_bmi, on=['age_group', 'school_year', 'depression_level'])
    
    print('Age Group, School Year, Depression Level, Count, and Mean BMI:')
    for age_group in age_group_depression_bmi['age_group'].unique():
        group = age_group_depression_bmi[age_group_depression_bmi['age_group'] == age_group]
        for school_year in group['school_year'].unique():
            subgroup = group[group['school_year'] == school_year]
            subgroup = subgroup.sort_values(by='counts', ascending=False)
            subgroup = subgroup.head(5)
            print(f'Age Group: {age_group}, School Year: {school_year}')
            for i, row in subgroup.iterrows():
                print(f'\tDepression Level: {row["depression_level"]}')
                print(f'\tCount: {row["counts"]}')
                print(f'\tMean BMI: {row["mean_bmi"]}')
            print('')

def anxiety_age_group_count(df):
    # Create the age_group column
    df['age_group'] = pd.cut(df['age'], bins=[17, 21, 25, 29, 100], labels=['18-21', '22-25', '26-29', '30 above'])
    df = df[df['anxiety_diagnosis'] == True]
    # Create the anxiety_level column
    df['anxiety_level'] = 'None-minimal'
    df.loc[df['gad_score'] >= 5, 'anxiety_level'] = 'Mild'
    df.loc[df['gad_score'] >= 10, 'anxiety_level'] = 'Moderate'
    df.loc[df['gad_score'] >= 15, 'anxiety_level'] = 'Moderately severe'
    df.loc[df['gad_score'] >= 20, 'anxiety_level'] = 'Severe'
    
    # Group the data by the age_group and anxiety_diagnosis columns
    age_group_anxiety_counts = df.groupby(['age_group', 'school_year', 'anxiety_level']).size().reset_index(name='counts')
    age_group_anxiety_mean_bmi = df.groupby(['age_group', 'school_year', 'anxiety_level'])['bmi'].mean().reset_index(name='mean_bmi')
    
    # Group the data by the anxiety_level and anxiety_diagnosis columns
    anxiety_level_counts = df.groupby(['anxiety_level', 'anxiety_diagnosis']).size().reset_index(name='counts')
    anxiety_level_mean_bmi = df.groupby(['anxiety_level', 'anxiety_diagnosis'])['bmi'].mean().reset_index(name='mean_bmi')
    
    # Calculate the mode of school_year for each age_group
    # age_group_mode_school_year = df.groupby(['age_group'])['school_year'].apply(lambda x: x.mode()[0] if x.nunique() > 1 else None).reset_index(name='mode_school_year')
    

    # Merge the two dataframes
    age_group_anxiety_bmi = age_group_anxiety_counts.merge(age_group_anxiety_mean_bmi, on=['age_group', 'school_year', 'anxiety_level'])

    # Print the results
    print('Age Group, School Year, Anxiety Level, Count, and Mean BMI:')
    for age_group in age_group_anxiety_bmi['age_group'].unique():
      group = age_group_anxiety_bmi[age_group_anxiety_bmi['age_group'] == age_group]
      for school_year in group['school_year'].unique():
            subgroup = group[group['school_year'] == school_year]
            subgroup = subgroup.sort_values(by='counts', ascending=False)
            subgroup = subgroup.head(5)
            print(f'Age Group: {age_group}, School Year: {school_year}')
            for i, row in subgroup.iterrows():
                print(f'\tAnxiety Level: {row["anxiety_level"]}')
                print(f'\tCount: {row["counts"]}')
                print(f'\tMean BMI: {row["mean_bmi"]}')
            print('')
